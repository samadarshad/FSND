...
class TriviaTestCase(unittest.TestCase):
    ...
    def test_get_questions(self):
        ...
        self.assertEqual(data['total_questions'],
                         len(expected_questions_formatted))
    ...
    def test_search_question(self):
        # this is question id 5 and 6 in the test database
        res = self.client().post('/questions', json={'searchTerm': 'title'})
        data = json.loads(res.data)
        expected_questions = [Question.query.get(
            5).format(), Question.query.get(6).format()]
    ...
    def test_get_questions_by_category(self):
        # questions in category 1 are id:20, 21 and 22
        res = self.client().get('/categories/1/questions')
        data = json.loads(res.data)

        expected_questions = [
            Question.query.get(20).format(),
            Question.query.get(21).format(),
            Question.query.get(22).format()]
    ...
    def test_404_if_quiz_category_nonexist(self):
        res = self.client().post(
            '/quizzes', json={'quiz_category': {'id': 1000},
                              'previous_questions': []})
        data = json.loads(res.data)
    ...
    def test_post_non_previous_quiz_question(self):
        # category 6 only has questions of id 10 and 11
        res = self.client().post(
            '/quizzes', json={'quiz_category': {'id': 6},
                              'previous_questions': []})
        data = json.loads(res.data)
